...
const GATEWAY_URL = 'https://v1.halsen.id';
...
async function harvestAll() {
  ...
  try {
    ...
    console.log("Fetching SLiMS OPAC catalog...");
    try {
      const slimsRes = await fetch(`${GATEWAY_URL}/api/v1/library/opac?limit=500`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (slimsRes.ok) {
        const slimsResult = await slimsRes.json();
        const slimsData = slimsResult.data || [];
        ...
        console.error(`⚠️ SLiMS OPAC API failed with status: ${slimsRes.status}`);
      }
    } catch (err) {
    ...
    console.log("Fetching EPrints Repository catalog...");
    try {
      const eprintsRes = await fetch(`${GATEWAY_URL}/api/v1/library/repository?limit=500`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (eprintsRes.ok) {
        const eprintsResult = await eprintsRes.json();
        const eprintsData = eprintsResult.data || [];
        ...
        console.error(`⚠️ EPrints API failed with status: ${eprintsRes.status}`);
      }
    } catch (err) {
    ...
    console.log("Fetching OJS Journals catalog...");
    try {
      const ojsRes = await fetch(`${GATEWAY_URL}/api/v1/library/journals?limit=500`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (ojsRes.ok) {
        const ojsResult = await ojsRes.json();
        const ojsData = ojsResult.data || [];
        ...
        console.error(`⚠️ OJS API failed with status: ${ojsRes.status}`);
      }
    } catch (err) {
      console.error("⚠️ OJS fetch failed:", err.message);
    }
